...
# 【契約 v6】★②狭い型 ―― ★★`record_only_patch`（★★入力は 文字5つだけ・★★門は 置く直前に 通す）
...
出所: **MGR 17:58**（★`def` が 複数行の 時に 壊した ／ ★`):` の 次から）／ **MGR 16:07**（★落ちた1本＝`already_recorded` ／ ★足す1行を 名指し）／ **MGR 15:47**（★`at` を 足す＝★実装の 位置を ★出す時に 書く ／ ★いま 0/86）／ **MGR 12:52**（★封印は通った ／ ★worker が 落ちた＝`SyntaxError` ／ ★足す 2行を 名指し）／ **MGR 12:42 裁定**（★『門を worker の 場所へ 置く』案は 採らない ／ ★門は ★置く直前に 足場が 通す）／ **MGR 08:50**（★①の門が 実物を 通した ／ ★②の 中身も 指定 ／ ★足す 中身は 形が 固定）
