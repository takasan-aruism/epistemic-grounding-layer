# 2DER 機構マップ — 接続 / 記帳 / 参照 / 進行

> **★`BUILD_ROLE: SUPERSEDED by 2DER_EXECUTION_ARCHITECTURE.md`（2026-07-27）**
> **本文書は正典ではない。** 内容は `egl/docs/2DER_EXECUTION_ARCHITECTURE.md` に統合した。
> **理由**: 作業指示書 §2.4「新しい正本を無断で作らない」／**正典を2本にしない**（本日ずっと潰してきた「2本目の口」と同型）。
> **参照する場合は統合先を見ること。** 以下は統合前の記録として残す。
