...
- **SLICE-8 general problem-ingest(LIVE, DE-0144)**: `autonomy/ingest.py` = **TAKA INPUT → 2DER → CLAUDE** を敷く最小 path。raw Taka problem → current-state 再構成 + `self_grounding` retrieval で relevant history → investigator handoff。**collect-context→Claude の spine は LIVE**、**intelligent middle(triage/detection/reconstruction/next-realization/headless-exec)は honestly MISSING/EXHIBIT-ONLY(fake しない、scheduler CLOSED NEGATIVE 明記)**。dogfood(Qwen swap)= MIXED value(premise を実測で訂正・measured history 先出し=改善 / localization は依然 Claude / retrieval 不安定)。**Home の本義 = 問題・発想・目的の入口**(QUESTION/RESULT_LOG/TASK/CORRECTION taxonomy は最終形にしない)。
- **DESIGN-ONLY / MISSING(plan §11):** SLICE-2 spec 機械再生成。**intelligent middle**: general triage / detection / **reconstruction(alternative realization frame)= 2DER の核心未所有**。headless senior executor。ingest→handoff の運用強制(現状 advisory)。
