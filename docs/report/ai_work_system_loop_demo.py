...
honest bridges(未実装を stub と明示):
- RRI Research Design → APPROVED_RQ_SET(RRI §21-29 未実装 = BRIDGE_STUB)。
- EGL KNOWLEDGE_PACKET emitter(EGL-side packet 生成 = 台帳から薄く抽出する BRIDGE)。
real junctions: DS packet(step7)/ RRI Context Binding+strategy(step5,8)/ EGL admission(step6)/ DW gate(step3-5)。
"""
import json
import os
import sys
...
def run_loop(request, snapshot, recent_utterances, conversation_length):
    ...
    admission = egl_rp.ingest_result_packet(result_packet, reverify=lambda: (True, "EGL re-verified loop demo (stub OK)"))
    adm = [f"{a['admission_status']}:{a['validation_target']}" for a in admission["admitted"]]
    step("EGL", "← RESULT_PACKET admit", f"{adm}  rejected={len(admission['rejected'])}")
    ...
    print("\n=== loop closed: DS → RRI → EGL → DW → EGL → DS ===")
    print("  real junctions: DS packet / RRI ContextBinding+strategy / EGL admission(独立再検証) / DW gate")
    print("  bridges(未実装・honest): RRI Research Design→APPROVED_RQ_SET / EGL KNOWLEDGE_PACKET emitter")
