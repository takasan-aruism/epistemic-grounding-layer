...
**status: `PREREG_v0.4.3 — round-5: STILL-OPEN (narrowed). ESCALATED to Taka`. 実装・run・freeze/hash はしない。**
**round-5(DE-0121)**: 自律 overclaim を clean に除去・N-parity closed・DEV∪VAL disjoint 確認（genuine progress）。だが **root cause が criterion/pool/template authorship へ一段上へ移動** — B は fitting **data**（DEV∪VAL）を制約するが **apparatus**（criterion/pool/template）を Claude が target 既知で著す。**kept 'transfer' claim がその knob に load-bearing**（§3 訂正参照）。§9↔§11 矛盾。terminators = **(A) target-blind authorship** / **transfer→capability-exhibit へ downgrade** / (C) blind control arm。**Taka 判断待ち。freeze しない。**
...
→ 機構設計が hard-core の breakthrough_structure を **calibration/selection に使わない**（data-disjoint）＝ routing/selection leak は閉じ、pure target-memorizer は DEV/VAL gate で排除。
**⚠️ round-5 訂正（DE-0121）**: ただし B は fitting **data** を制約するが fitting **apparatus**（criterion rule/θ・ref-pool composition・R2 generation template；§11 ⟨TC⟩#1/#2）は依然 **Claude（target 既知）が著す** ＝ **criterion-authorship residual が残る**。DEV/VAL gate は steering を dual-purpose 演算子＋pool/template 選択へ **narrow するが除去しない**。**この residual は autonomy でなく transfer(held-out/generalization) claim に load-bearing** で、§1 の downgrade では discharge されない（旧記述『downgrade で contamination を honest に負う』は **transfer に対し false、撤回**）。**§9『no-Claude-discretion』は §11 が criterion を ⟨TC⟩ に残す点と矛盾**。terminator = authorship の decouple（A target-blind authoring）or claim を **capability-exhibit** へ更に downgrade。**Taka 判断待ち（§1/§7 の transfer は本 residual を負う）。**
