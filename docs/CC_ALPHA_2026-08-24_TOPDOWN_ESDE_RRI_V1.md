...
# 4.2 ★Phase 3 の実測 —— Top-Down が Bottom-Up を1つ潰した

★Phase 3 の土台（`docs.python.org` の登記・受入4/4）は済んだ。ここは**その先**の実測。

## 4.2.1 Bottom-Up —— 今日の失敗を「記録から」数え直した（★記憶で俯瞰しない）

| 数えた面 | 件数 |
|---|---|
| commit の表題（5 repo・本日） | **2件** |
| **台帳 `CHANGE_LOG` の試験欄**（本日 35 change 中） | **4件（11.4%）** |

★**面によって数が違う。** commit の表題は失敗の型を持たない ∴ **台帳の試験欄が正しい分母**。

該当 4件: `CHG-0229` / `CHG-0232` / `CHG-0235` / `CHG-0238` ——
**全部が Python の import 解決**（`sys.path` / `subprocess` / stdlib 遮蔽）。

★★これは仕様 §25「昇格条件」を**観測で**満たしている:
「同種が複数回発生」= **1日で4件**。★人が領域リストを書いたのではない。

## 4.2.2 系統は2つ在った（★1つに数えない）

| 系統 | 中身 | 該当 change |
|---|---|---|
| **A** | `/home/takasan/twoder/operator.py` が **stdlib の `operator` を隠す** | `CHG-0238` |
| **B** | subprocess で `from twoder import …` が引けない（**path が足りない**） | `CHG-0229` / `0232` / `0235` |

★**向きが逆**。A は path から**外したい**、B は path に**足したい**。

## 4.2.3 ★Top-Down の知識で A が消えた（実測）

Python **3.11+** の `-P` / `PYTHONSAFEPATH` は、`sys.path[0]`（script の置き場）を**入れない**。
★環境は **Python 3.13.5**（実測）∴ **使える**。

| 系統 | 素のまま | `-P` を付ける |
|---|---|---|
| **A**（twoder の `test_*.py` 6本） | **6本とも rc≠0** | ★**0本**（全部通る） |
| **B**（明示 `sys.path.insert` 経由） | rc=0 | rc=0（★**壊れない**） |

★★**`-P` は A を完全に消し、B を壊さない。**
★B が壊れない理由 = `-P` は **明示的に足した path を消さない**。

★★∴ **Top-Down（版に紐づく公式仕様）が、Bottom-Up の失敗系統を1つ潰した。**
★これが仕様 §13「TOP-DOWN × BOTTOM-UP」の**最初の実例**。★予想ではなく実測。

## 4.2.4 ★ここで `version_scope` が無いことが効いた

この知識は **「Python ≥ 3.11」という版に紐づく**。
★しかし `version_scope` は **0件**（§1.1 MISSING）∴ **記録する場所が無い。**

★∴ いまこの事実は **この文書にしか無い**。★次に同じ罠を踏む人へ機械では届かない。
★これが素案 §7 の要求が**実測で**必要になった瞬間（★予想で足していない）。

## 4.2.5 渡す先（★私は直さない）

- **`domain_sysops.sysops_failing_tests`（EVO-0102 担当）** ——
  `subprocess.run([sys.executable, p], cwd=root, …)` に **`-P` を足せば**
  `repo="twoder"` の偽の失敗 6/6 が消える（★実測）。
  ★但し `-P` は **3.11+ 限定** ∴ 版を確かめてから。
- **私の `domain_esde` の Worker 起動** —— 系統B なので `-P` は不要（明示 insert 済）。
  ★但し **付けても壊れない**ことは確認した。
