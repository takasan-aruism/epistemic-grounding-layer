...
## 9-2. ★★訂正(22:0x): ★★9-1 の ★出所の見立ては ★★誤りだった（★計器のソースを読んで確定した）

```
★★★私は 9-1 で ★「出所は commit 時刻である」と書いた。★★★これは ★誤りである。★取り下げる。
★★正しい出所（★計器のソース逐語・`~/.claude/hooks/2der_status.sh:104`）:
   `pstart=$(stat -c %Y /proc/"$pid")` ／ `find "$d" -name '*.py' -newermt "@$pstart" -print -quit`
   ★★`stat -c %Y` は ★★秒までしか返さない（★小数を切り捨てる）。
★★★実測（★同じ判定を再現した）:
   ★`twoder/webui.py` mtime = ★2026-08-01 20:19:02.★006112240
   ★`/proc/853291`   mtime = ★2026-08-01 20:19:02.★055000042  ← ★★プロセスの方が ★49ms ★新しい
   ★★★しかし ★計器は ★`@1785583142`（＝20:19:02.★000000）と比べる ∴ ★`.006` が ★「新しい」と判定される
★★★★★∴ ★★★偽陽性の原因は ★★秒の切り捨て であって ★★commit 時刻ではない。
   ★★★★MGR が `EVO-0033` の note に書いた「mtime とプロセス起動時刻の比較が粗い」が ★★正しい。
★★★★★★★★結論そのもの（★★再起動は要らない）は ★変わらない。★★ファイルは ★プロセスより ★49ms ★古い。
★★★★★★★★★私の誤りの形: ★1つの観測（★HEAD 時刻が進んでいた）で ★出所を断定した。★規律「1回の観測で断定しない」に反した。
```
