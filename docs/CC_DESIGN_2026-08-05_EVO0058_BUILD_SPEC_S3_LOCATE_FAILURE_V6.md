# 【BUILD SPEC v6】`EVO-0058` 段3 — **★どの区間で止まったかを返す。★言えない区間は言えないと返す**
...
- **★v5 からの差は ★依頼文の `actor` の書き方だけ**: ★★試験は ★1本も緩めない・★1文字も変えない（★落ちた2本も ★残す）
- **★`enable_thinking=False` は ★入ったまま**（★戻さない）
...
・actor / actor_known は 次のとおり。★どの行から引くかを ★間違えないこと:
    LOCATED     → ★segment と ★同じ "id" を持つ route の行の "actor"。actor_known は True。
    BOUNDED     → ★★candidates[0] と ★同じ "id" を持つ route の行の "actor"。actor_known は ★False。
                  ★★candidates[0] は ★id の文字列である ―― ★route の添字として使わない。
                  ★★★last_observed の行ではない。★route の先頭の行でもない。★id で引くこと。
    NO_EVIDENCE / NO_FAILURE → actor は None。actor_known は False。
