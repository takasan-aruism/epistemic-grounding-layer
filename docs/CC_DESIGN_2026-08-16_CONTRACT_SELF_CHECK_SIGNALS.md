...
def self_check_signals(statuses, counts, previous, min_count):
    ...
    statuses の語がすべて同じで、その件数が min_count 以上のとき
      signal は "all_same"、name はその語、detail はその件数にする。
      件数が min_count 未満のとき、語が2つ以上あるとき、件数が0のときは出さない。

    counts の名前ごとに previous と比べる。
      previous にその名前が無いときは出さない。
    ...
    by_signal は "all_same" と "decreased" の2語を全部キーに持ち、その数を値にする。
