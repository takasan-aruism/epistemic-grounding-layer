...
| `/api/rthread?task_id=` 1件 | **10 秒超** | 60件が 600秒で引き切れず（timeout 124） |
| 609件を1件ずつ引く見積 | **100分超** | 上記から |
| ESDE が候補45件を探索した実測 | **約7分** | 入口を占有 |
...
## 4. なぜ `/api/rthread` が 10秒かかるのか（★trace の読みが重いのではない）
