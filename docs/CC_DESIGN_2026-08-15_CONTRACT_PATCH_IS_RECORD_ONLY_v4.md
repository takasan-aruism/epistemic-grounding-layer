...
# 【契約 v4】★`patch_is_record_only` ―― ★★注釈の行 と ★呼び出しの続きの行 を 許す

宛: MGR ／ 写: IMPL ／ 発: DESIGN ／ 2026-08-15 08:5x ／ 台帳: `ITEM-2DER-EVO-0058`
差し替え元: `…_PATCH_IS_RECORD_ONLY_v2.md`（★中身は 触らない＝★新しい名前）
出所: **MGR 17:58**（★門が 壊れた計装を 通した ／ ★`compile` を 足す）／ **MGR 08:30**（★実物の 差分で 落ちた ／ ★足す 2行を 名指し ／ ★★『同じ試験に 3回目』では なく ★新しい事実への 1回目）
...
def patch_is_record_only(before, after, allowed_calls=None):
    ...
    ok は次が全部成り立つときだけ True。
      ...
      after が Python として読める。compile で確かめる。読めなければ ok は False。
    ...
    reasons は ok が False のときの理由の語の一覧。次の語を使う。当てはまるものを全部入れる。
      "removed_line" "changed_line" "not_allowed_line" "def_changed" "return_changed" "syntax_error"
...
def test_broken_syntax_is_not_ok():
    """読めない本文は通さない。行の増減だけで判断しない。"""
    b = "def f(x,\n        y=None):\n    return 1\n"
    a = "def f(x,\n    emit_record()\n        y=None):\n    return 1\n"
    r = patch_is_record_only(b, a, allowed_calls=["emit_record"])
    assert r["ok"] is False
    assert "syntax_error" in r["reasons"]
