...
| 指示語の表層一致のみ | **【訂正】2/298 = 0.67%**（初版の「2.0% (6/298)」は誤り。6件中4件は裸の `この` の部分一致による偽陽性） | 3/3 |
...
7. **【訂正】cross-repo は `rri` / `twoder` / `egl` の3 repo に跨る**（初版の「rri 単体」は誤り）。**3 repo それぞれ commit + push**（[[2der_repo_topology]]）。
