# 【BUILD SPEC v2】`EVO-0058` 段3 — **★どの区間で止まったかを返す。★言えない区間は言えないと返す**
...
- **★v1 からの差は2つだけ**: ①★`S11` を試験に入れた（★MGR の指摘）／②★受入(3)(b) を ★MGR の裁定文へ差し替えた
...
**★封印試験（★15本・★fixture は §2 の実測。★意図ごとに1本）**
```
...
```
★(1) ★worker が `locate_failure` を書く（★Claude は本文0行・★実行記録で確認）／★(2) ★15本 全通
