...
- **gate(順序は load-bearing・崩さない)**: HBB N=13 closure(二軸 consensus)→ **robust hard-core 固定** →
  ...
  **HBB N=13 closure 完了(DE-0115)**: robust hard-core = **{HBB-08, HBB-10, HBB-30}** に固定済(`hbb_hard_core_fixed.json`)。
  これが R1-R4 / STOP-SHIFT-RUN-COMPARE の ground truth。狙う欠落 = **autonomous(H0)reconstruction**(現状 no arm、engine 優位は H1 hint-assisted のみ)。
- **prereg DRAFT 作成済(v0.3, R1–R5)**: [[hbb_reconstruction_prereg_v0.3_draft]](`PREREG_DRAFT — AWAITING_TAKA`)。
  v0.3 の manifest multipass / extensible-with-refs / STRUCTURAL PROJECTION selector / MULTI_SCORER_CONSENSUS /
  token-budget matched を反映。scheduler 内部仕様は `⟨TC⟩`(Taka 確定待ち)。実装・run は go 待ち。
