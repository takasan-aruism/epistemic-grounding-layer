# 【BUILD SPEC v3】`EVO-0035` — **★書き先の名前を検査する（★止める／★既定へ落とさない）**
...
- **★v2 からの差は ★試験を ★もう1本 足しただけ**（★依頼文は ★1文字も変えない・★既存9本は ★1本も緩めない）
- **★足した理由**: ★v2 は ★全角しか 縛っていない ∴ ★★全角だけを 特別扱いする実装が ★10本 全通する
  （MGR 実測・★アラビア数字 `٣` と 上付き `²` は ★依然 通る）
- **★v1 からの差は ★試験を1本 足しただけ**（★依頼文は ★1文字も変えない・★既存8本は ★1本も緩めない）
- **★足した理由**: ★依頼文には「★半角の」と書いたが、★★`str.isalnum()` は ★全角も True を返す
  ∴ ★★その実装が ★10本 全通してしまう（★本日 何度も踏んだ形＝★規則は書いた／★試験には書かなかった）
...
def test_fullwidth_is_rejected():
    """★半角だけを 通すこと（★`str.isalnum()` は ★全角も True を返すため ★試験で 縛る）"""
    assert impl.is_valid_suffix("ｖ１") is False

def test_non_ascii_letters_and_digits_are_rejected():
    """★半角の英数字だけを 通すこと。★`str.isalnum()` は 次を すべて True にする
    （★全角・アラビア数字・上付き・ローマ数字・かな・漢字・数学英数記号）∴ ★まとめて 縛る"""
    for s in ("\uff56\uff11", "\u0663", "\u00b2", "\u2163", "\u3042", "\u6f22", "\U0001d7d9"):
        assert impl.is_valid_suffix(s) is False, s
