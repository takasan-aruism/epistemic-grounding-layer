# 【BUILD SPEC v3】`EVO-0058` (C') — **★渡し方の事故を除外する `reconcile_handoff`**
...
- **★v2 からの差は ★試験を1本 足しただけ**（★14本 → ★15本）。★既存14本は ★1文字も変えない
- **★依頼文は ★v2 から ★1文字も変えない**（★MGR 逐語「★規則は正しい・★試験が足りない」）
- **★v1 からの差**: ★依頼文の④⑤の書き方（★v2）＋ ★試験1本（★v3）。★★測る強さは ★上がるだけ・★下がらない
...
**★封印試験（★15本・★fixture は ★2026-08-05 の実測から取る）**
```
...
def test_no_event_is_reported_as_broken():
    """★配線済みの区間で ★何も起きていない = ★★断線として ★数える（★未配線ではない）"""
    v = impl.reconcile_handoff([R14], [])
...
## 2-c. ★★v3 で足した1本（★私の欠落・★MGR 指摘）

```
★★『★配線済みなのに ★何も起きていない(NO_EVENT)』が ★★`broken` と `first_break` に ★出ることを
   ★★1本も縛っていなかった ―― ★`link` の語だけ見て ★★集計側を見ていない。
★★★これは ★本日 何度も踏んだ形（★依頼文には書いた／★試験には書かなかった＝v1.11）。
★★★★依頼文の逐語は ★正しい =「★broken = link が "LINKED" でも "NOT_WIRED" でもない行の id」
   ∴ ★★規則を変えない。★★足りないのは ★試験だけ（★MGR の逐語と一致）。
★★★★★意味 = ★★NO_EVENT は ★『まだ配線していない』ではなく ★『配線したのに ★通らなかった』
   ＝ ★★断線であり ★数えなければならない。★NOT_WIRED と ★混ぜない（★既存1本が ★そちらを縛っている）
```
...
```
★(0) ★worker が書く（★Claude は本文0行）・★15本 全通
