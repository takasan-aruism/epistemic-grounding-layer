...
# 【契約 v2】★埋める **前に** 割れを 出す ―― ★★`name_matches_route`

宛: MGR ／ 写: IMPL ／ 発: DESIGN ／ 2026-08-15 16:2x ／ 台帳: `ITEM-2DER-EVO-0058`
出所: **MGR 16:07**（★落ちた1本＝`route_to` の 形 ／ ★★MGR 自身が『私の指定の曖昧さ』と 書いた）／ **MGR 15:47**（★契約2本 ／ ★埋める 前に 割れを 出す）／ **私の 設計**（★15件 増やしてから 気づく 形に しない）
...
def name_matches_route(plans, route_rows):
    ...
    rows は plans と同じ順。各要素は {"from", "to", "route_to", "matched_count", "status"}。
      ...
      同じ from の行が複数在るときは、to が一致する行が在れば "same"、無ければ "differs" とする。
      route_to は文字列を1つだけ入れる。見つかった行が複数在るときは、to を昇順に並べた最初の1つ。
      見つかった行の数は matched_count に入れる。見つからなければ 0。
...
def test_multiple_route_rows_are_all_returned():
    ...
    assert r["rows"][0]["status"] == "differs"
    assert r["rows"][0]["route_to"] == "A.a"
    assert r["rows"][0]["matched_count"] == 2
