# 【BUILD SPEC v4】`EVO-0049` — **★『分からない』を signals に載せる（★列挙は増やさない）**
...
- **★v3 からの差は ★docstring ★1行だけ**（★前提を書き足した。★`assert`・★名前・★fixture は ★1文字も変えない）
- **★足した理由**: ★その1行が ★前提を書かずに ★★一般則として読める形だった（MGR 実測・★4標本すべてで ★同じ1本が落ちた）
