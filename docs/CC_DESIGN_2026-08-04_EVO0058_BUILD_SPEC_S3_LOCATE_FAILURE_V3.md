# 【BUILD SPEC v3】`EVO-0058` 段3 — **★どの区間で止まったかを返す。★言えない区間は言えないと返す**
...
- **★v2 からの差は1つだけ**: ★`actor` と `segment` が **★route 由来である**ことを ★試験で縛った（★4本 追加）
...
**★封印試験（★19本・★fixture は §2 の実測。★意図ごとに1本）**
```
...
```
★(1) ★worker が `locate_failure` を書く（★Claude は本文0行・★実行記録で確認）／★(2) ★19本 全通
