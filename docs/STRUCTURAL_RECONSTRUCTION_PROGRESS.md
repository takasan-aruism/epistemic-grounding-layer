...
`s1e_executed.py` / `s2_extract.py` / `s2b_consensus.py` / `s3_components.py` / `s3b_item_ladder.py` / `s4_edges.py`

未生成: `CLAIM_EVIDENCE_INDEX` / `HISTORY_EVENTS` / `CONTRADICTIONS`
...
**Stage 5: HISTORY_EVENTS + フェーズ分割**（決定論主体）。

Stage 4 までで「いま何が繋がっているか」は確定した。残るのは「いつ、なぜそうなったか」である。
辺の生成・消滅を commit 単位で差分化し、DE / ITEM と結線する。

その後 Stage 6（統合・矛盾監査・系譜監査）、Stage 7（TR-1..6）、人間向け 4 本。
