...
`s1e_executed.py` / `s2_extract.py` / `s2b_consensus.py` / `s3_components.py` / `s3b_item_ladder.py` / `s4_edges.py` / `s5_history.py` / `s6_contradictions.py`
...
**Stage 7: トレーサビリティ試験 TR-1..6**（決定論）。TR-4 は §3.5 で先行合格済み。
続いて `CLAIM_EVIDENCE_INDEX.jsonl`、人間向け 4 本、`twoder/audit/` 07-12 版との diff。

Stage 7 で TR-2（任意の ITEM → コードと実行記録）に合格するには U10（ITEM 束縛 44/86）を
閉じる必要がある。`CHANGE_LOG.jsonl` / DE の `affected_artifact_ids` から解決する。
