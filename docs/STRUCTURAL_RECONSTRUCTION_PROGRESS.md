...
`s1e_executed.py` / `s2_extract.py` / `s2b_consensus.py` / `s3_components.py` / `s3b_item_ladder.py` / `s4_edges.py` / `s5_history.py`

未生成: `CLAIM_EVIDENCE_INDEX` / `CONTRADICTIONS`
...
**Stage 6: 統合・矛盾監査・系譜監査**。

決定論層は Stage 1〜5 で出揃った（LLM を使ったのは Stage 2 のみ）。残りは:

- `CONTRADICTIONS.jsonl` — spec v0.1 §6.1 の 7 種のうち **6 種を機械計算**
  （`ROADMAP.status=DONE ∧ wired=NO` を v0.2 で機械側へ移した）。
  LLM に残るのは「同一コンポーネントの記述不一致」1 種のみ。
- `CLAIM_EVIDENCE_INDEX.jsonl`
- Stage 7 の TR-1..6（TR-4 は §3.5 で先行合格）
- 人間向け 4 本 + `twoder/audit/` の 07-12 版との diff
