...
`s1e_executed.py` / `s2_extract.py` / `s2b_consensus.py`

未生成: `COMPONENT_INVENTORY` / `EDGE_INVENTORY` / `CLAIM_EVIDENCE_INDEX` / `HISTORY_EVENTS` / `CONTRADICTIONS`
...
**Stage 3: COMPONENT_INVENTORY**。入力は以下に限る:

- 決定論層（FILE_MANIFEST / SYMBOL_INDEX / REACHABILITY / SYMBOL_REACHABILITY / EXECUTION_EVIDENCE）
- **`FILE_EXTRACTION_CONSENSUS.jsonl` のみ**（生の単一シード出力は使わない — I3）
- `authority_checks` は LLM 出力を使わず `twoder/authority.py` の決定論的解析に置き換える（U8）

Stage 3 の入口で**陰性対照**を先に通すこと（U9）。

未決のまま進行中: §4 U5 の Taka 裁定 4 点。暫定予算は Claude が置いた値のまま
（生成 200 万 tok / 1,800 s、現在 64.6% / 30% 消費）。
