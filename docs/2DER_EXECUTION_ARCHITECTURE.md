...
| **G-62** | 「取り下げ済み」と書かれた制約がコードに残っている（account 保存則 `D3`） | Gap | 未着手（★D-52 で発見） |
