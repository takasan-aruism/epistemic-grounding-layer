...
| **G-45** | 本番未到達の台帳書き手**2件**（`request_thread` / `dw.authorization`） | Gap | **DEFERRED**（★当初3件としたが1件は到達すると判明し訂正） |
...
| **G-49** | `acquisition.py` 固有の境界イベント（実装監査用 Instrumentation） — **★2026-07-30「不要」と決めた**（★「やらない」ではなく**「不要と決めた」**） | 設計判断 | **CLOSED**（★D-93 裁定⑤。理由=完了像6項目に出てこない／Phase 1 の Taka 裁定で除外済／Phase 2 の条件にも入らない） |
| **G-50** | 入口一覧が不完全だった — `dw/workflow.py:37` が**13番目**の入口 — **★2026-07-30 述語を1つ決めて1回走査し総数を記録（構文木・全5repo・打ち切り無し → ★総数88件／テスト59・本番16・実験13）** | Gap | **CLOSED**（★D-92 裁定③。**14番目を探しに行かない**。★「本番16件」を**「16個の入口」と書かない**——数えただけ） |
| **G-51** | RRI の終了条件 — 定義は例外で強制されているが本番から一度も呼ばれない | Gap | 未着手（★既定義・未接続） |
| **G-52** | RRI への再入 — 2機構が別々に在り互いを知らない。`RESOLVED` からの再入は無い | Gap | 未着手（★部分定義） |
| **G-53** | RRI の状態と正規保存場所 — 定義された場所と本番が書く場所が違う | Gap | 未着手（★既定義・不一致） |
| **G-54** | Event Trace → RRI — id は辿れるが判断・根拠・残差は辿れない | Gap | 未着手（★部分定義） |
| **G-55** | `request_thread.py` は正典で halt も解けているのに本番から一度も呼ばれない — 理由は未解明 | Gap | **DEFERRED**（★中心が終わってから） |
| **G-56** | DW task を作らない依頼は管理UI で開けない（`no such task`） | Gap | 未着手（★D-49 の受入に影響・裁定待ち） |
| **G-57** | `run_id` を直接入れて event 列だけ見る手段が無い | Gap | **DEFERRED**（★今日の run を画面で見るには要る） |
| **G-58** | submit ごとの trace を読む口が無い（**残っているが読めない**） | Gap | **DEFERRED**（★`G-31` の理解を更新する材料） |
| **G-59** | **HALT 後のライフサイクル管理が無い**（解けたことが伝わらない／止まっていることが見えない） | Gap | 未着手（★Phase 2 の項目・2DER 全体の運用基盤） |
| **G-60** | 「1文字も変えない」を運ぶ手順が無い（`$(cat)` が末尾改行を落とし別 task になる） | Gap | 未着手（★本日2度目の実害・CC-α の中継落ち度） |
| **G-61** | 「`task_id` が複数在れば止める」分岐は実装済だが**未検証**（該当 run が存在しない） | Gap | 未着手（★作って試さない） |
| **G-62** | **5.** 取り下げ済みmass保存則の実装残存（★Taka 未完案件6項目の5番と同一） | 設計欠落 | 未着手（★実装しない・配線しない） |
| **G-63** | 科目を作る側と使う側が別名で設計され、繋ぐ1段が未実装（**変質ではなく未完**） | Gap | 未着手（★Taka 指摘の裏づけ） |
| **G-64** | **1.** ACCOUNT_AXESからRThread chartへの正規変換仕様 | 設計欠落 | 未着手（★Taka 未完案件） |
| **G-65** | **2.** chartのversioning・生成責任者・保存場所 | 設計欠落 | 未着手（★Taka 未完案件） |
| **G-66** | **3.** request_threadが読むchartとの同一性 | 設計欠落 | 未着手（★Taka 未完案件） |
| **G-67** | **4.** 初回chartが存在しない状態の起動手順 | 設計欠落 | 未着手（★Taka 未完案件） |
| **G-68** | **6.** MINING→chart→RRI→LedgerまでのExecution Architecture | 設計欠落 | 未着手（★Taka 未完案件） |
| **G-69** | 「欠損・失敗の記録なし」が2つの場合を1文言でまとめていた — **★2026-07-30 実測で★既に分かれていることを確認**（D-58 で分割済・2文が別に出ている） | Gap | **CLOSED**（★D-93 裁定③。**MGR が新しい作業を作らずに済んだ**。★Taka 残件④は**これで済**。★実例は**原理的に出せない**ので条件にしない） |
| **G-70** | サーバが `/api/submit` を受けたことを1行も記録していない（「届いていない」と「断った」を区別できない） | Gap | **PARTIALLY_CLOSED**（★`RECV` は実際に出た。成功時 `SENT` は未確認） |
| **G-71** | **受入が「壊れていても必ず通る」形だった**（文字列検索は死んだページでも同じ答えを返す） | 作法の欠陥 | 未着手（★`node --check` を受入に足す） |
| **G-72** | **155件中18件(12%)で「次に誰が何をするか」の答えが「誰も」** — **★原因が分かった: 18件はすべて `JUDGE_REQUIRED`（裁定が要る）で、`ADJUDICATOR` は宣言だけ在り担当者が割り当てられていない** | Gap | 未着手（★**原因は判明・直さない**——誰がやるかは我々が決める話ではない。Taka へ YES/NO を照会中） |
| **G-73** | 一覧に出せる**短い名前が無い**（`goal` は依頼文そのもの・中央値3816字） | Gap | 未着手（★要約は新しい能力・今やらない） |
| **G-74** | 非回帰98本の走らせ方を版管理された場所に残す — **★2026-07-30 取り消した**（★走らせ方を残すことは**常設命令に反する操作の手順を残すこと**だった） | 作法の欠陥 | **CLOSED**（★D-106 §4-3。★代わりに **`G-77`** に「なぜ走らせないか」を残した） |
| **G-77** | **★非回帰98本のうち2本が、走らせるたびに本番の front door へ★架空の依頼文を POST する**（1走行で8回以上）— **★Taka 常設命令に反する** | 作法の欠陥 | **OPEN**（★**走らせるのをやめた。直していない**。★受入が1つ弱くなることを承知で止めた。★Taka の判断待ち） |
| **G-75** | **Taka 残件③「DW枝の到達性確認」** — **★2026-07-30 述語を「宣言された状態(13)」と定めて1回走査: ★到達9 / ★到達した記録が無い4**（`AUDIT_FAILED`/`IMPLEMENTING`/`PLANNING`/`REWORK`） | Gap | **CLOSED**（★D-93 裁定②。**「確認した」は「全部 到達した」ではない**。★未到達は**「到達できない」ではなく「記録が無い」**。★到達させに行かない） |
| **G-76** | **Taka 残件⑦「独立検算の正式な定義」が★資料に登記されていなかった**（登記漏れ・CC-α の責）— **★D-60 の判定は「済（D-59）」** | Gap | **CLOSED**（★2026-07-30 B-4 で訂正。**CC-α が付けた `DEFERRED` は誤り**——D-60 を読まずに登記した。★定義は D-59 の1本の裁定であり「正式な定義文書」は無い） |
| **G-46** | **TRACE に置いた値57件のうち★24件が front door から読めない**（★2026-07-30 実測: 35→34件、うち別経路が在る9件を除いて24件）— **★`ETRACE_RUN_ID`（Taka 残件②）は済** | Gap | **PARTIALLY_CLOSED**（★残る**24件は【C】**＝Phase 2 の量ではない。★「取れない」ではなく**「見つからない」**——見ていない口が2つ在る） |
| **G-78** | **状況表が「台帳のずれ」を出した★直後に MGR 未応答の一覧を出すため、★ずれの内訳に見える**（Taka 指摘・D-124 §5(a)） | instrument_defect | **OPEN**（★**直さない。登記して、誤用しない**——Taka「修正より先に、今後の判断で誤用しないことを優先します」。★`2der_status.sh:90` の直後 `:91-93` が別計算の `$PENDING` を字下げして出している。★`head -8` の打ち切りもある） |
...
| **G-84** | **★`submit()` に渡る受理時刻 `ts` が既定値 `2026-07-11T08:00:00` に落ちている**（`twoder/submit.py:93-94`。★実時刻は Event Trace 側が持っていた） | gap | **OPEN**（★**直さない・提案もしない**。第2試行の `SUBMIT-` 解決は `ETR-` の `ENTRY.ts` を使って**迂回できた**が、★**迂回できたことを「問題が無い」と読まない**。★`D-141` §4 の指示により設計が採番） |
| **G-85** | **★状況表が表示する稼働 webui の PID が実サーバと食い違っていた**（表示 `3814626`＝**親シェル** ／ 実体 `3814663`＝本体） | instrument_defect | **OPEN**（★**直さない**。★IMPL が確認せず `3814626` を kill し、実サーバは無傷・再起動は空振り＝**自己申告**。★**「実行中プロセス」を信じて kill すると別物を殺す**。★§4-15 の隣接形＝**親シェルと本体を区別していない**） |
...
| **G-91** | **★受入3 に当たる指示が、PLAN の最終 prompt に ★1件も無い** — `do not re-`/`re-acquire`/`again`/`skip`/`avoid`/`duplicate`/`must not`/`do not build` **すべて 0件**（★変数展開後の全文 3705字・sha1 `8c01ef84…`・大小無視・打ち切り無し）。`already` 5件の内訳は★見出し3・欄定義2（＝**列挙の指示であって禁止ではない**） | gap | **OPEN**（★**我々は書いていない規則で 40本 採点していた**。★A/B/C1/C2 各10本で「追加作業不要」は **0/38**＝実行欄・作業欄・観測の絞り、**3軸とも動かなかった**。★依頼文自身が取得手段を求めている ∴ **不足を取りに行くのは依頼の履行**。★最小修正案 9-a＝**採点をやめる・コード変更0**） |
| **G-92** | **我々が「制約」と呼んできた4要素は Qwen に一度も渡っていない** — **system prompt 0件**（`build_plan` は `qwen_raw_call` に `system` を渡さない ∴ `messages` は user 1件のみ）／**validation 条件 0件**／**再生成条件 0件**／**Audit へ渡す内容 0件** | gap | **OPEN**（★「指示したのに従わない」と読むと**因果を取り違える**。★`validate_plan`/`run_with_recovery` はいずれも**後段のコード**で prompt に現れない） |
| **G-93** | **Observation ブロックは強制語ゼロの平叙文** ＝ prompt 内で**構造上いちばん弱い入力**（強制語は計7件で**すべて出力の形か作り方の制約**に付く） | gap | **OPEN**（★観測**値**の引用は **2/38**、**手段**は 31/38。★`already_satisfied` の器を1つ足したら **38/38 で埋まった**＝**器は効くが、書かれるのは手段であって値ではない**。★案 9-b は依頼文と正面衝突しうる ∴ **9-a の後でないと効果を測れない**） |
| **G-94** | **【将来必須機能・正典登録】自己改善ループ**: 監査結果 → 原因分析 → 修正候補生成 → 上級監査AIコメント → **人間承認** → 機能修正 → Ledger 登録 → 同種失敗への再利用 | planned | **REGISTERED_NOT_BUILT**（★Taka 正典「**正典登録のみ行ってください**」／★D-175 §6 の宿題をここで果たした。★着手条件は G-91 の定義確定より後。★**人間承認の段が入っている ∴ 自律 RD の有効化ではない**） |
| **G-95** | **規則を1文 足すと出力が伸びて打ち切りが増える** — 同一 task・同一 prompt から**規則の1文（376字）だけ**を抜いた対照と比較して、`finish_reason=length` **1/10 → 4/10**、出力トークン中央値 **4100 → 7524** | gap | **OPEN**（★**`G-88` に当たる ∴ `max_tokens` を上げて隠さない**。★率は 24.1%→16.7% と下がったが、**打ち切り4本の除外＝生存者バイアスの向きと一致する**ので根拠にならない。★**打ち切り4割のままでは何を測ってもバイアスが乗る** ∴ 率の議論より先に打ち切りを下げる。★誰も予告していなかった） |
| **G-96** | **物差しの母数を、採点される側が自分で決めている** — 規則は「above に挙がっているもの」を指すが、`already_satisfied` に何を挙げるかは**計画自身**が書く | gap | **OPEN**（★同一 prompt で母数が **12 対 29 ＝ 2.4倍** 違った。★**モデルではなく物差しの弱点**。★仮説（未確認）: 規則を強くするほど `already_satisfied` を短く書く動機が生じる——実測の向きは一致するが 20本では確かめられない） |
| **G-97** | **同一 prompt・同一設定でも n=10 の1バッチでは打ち切り率が動く** — 376字規則入りの**同一 prompt**（temperature 0 / max_tokens 8192 / 同時実行10 すべて同一）を3回投げて **打ち切り 4/10 → 2/10 → 2/10**、**出力token 中央値 7267 → 4020 → 4891** | gap | **OPEN**（★これにより `D-183`/`D-184` の「**打ち切りが4倍・思考が倍近く**」は**再現せず取り下げた**——CC-α の誤りで Taka へ上がった数字。★合算では方向のみ残る（376字 8/30=27% 対 規則なし 4/30=13%）。★**作法**: 対照は必要だが十分ではない、**反復が無ければ大きさを書けない**。★**temperature 0 だから決定論、と書かない**） |
| **G-98** | **純粋な肯定形の規則（案B）は打ち切り・完成率・schema で最良だが、「不足の取得方法を提示しない」計画が 32% に増える** | gap | **OPEN**（★打ち切り 13%/27%/**5%**、token中央値 5026/6019/**3618**、schema不成立 5/8/**1** ＝ 案B が最良。★しかし**取得方法を出さず 12%/14%/★32%**。★**確定30本だけでは 2/1/1 で見えず、追加30本の合算で初めて見えた**。★設計の予想した副作用（as記載が減る）は外れ、出たのは予想していない側。★MGR の懸念1（過剰修正）に直接 当たる） |
...
---
*2DER Execution Architecture v0.1（常設・正典）。`2DER_MECHANISM_MAP.md` を包含し統合する（正典を2本にしない）。★要約=書く側も読む側(`GET /api/resolve`)も繋がっており、繋がっていないのは決定論で絞る側である（★2026-07-28 訂正: 初版は「読む側が繋がっていない」「最大の切断は G-01」と書いたが**誤りだった**——探索範囲を `submit()` の acquisition 枝に限っており、別の口を見ていなかった。G-01 は CLOSED）。勘定科目は EGL 登録経路に無く(G-02)、4軸→7戦略の決定論選択は既定で入らない分岐にあり3軸は本番で生成されない(G-03)。PLAN は Qwen が書き(決定論テンプレは front door 由来 task では原理的に発火しない)、worker は契約(skeleton+immutable_tests)が無ければ着手せず、契約は依頼者(Claude)が渡す。worker は production repo に三重の保証で書けず、配置は Claude の役割。Gap Register は G-01〜G-77（★2026-07-30 Phase 2 の**実装**が終わった——**『Phase 2 完了』ではない。残件⑤(b) は待ち**。★本日=**155行の ID → 6行の件数**（母数153・`BLOCKED` は外）／**人称語を使わず名指し**／**2種類の『無い』を分けた**（`uncountable` は測れて0 ／「Taka の手番」は測れない）／**内訳は固定文にせず `state` を数える**／**集計の対象外（止まった2件）を『開いて読める』ようにした**（母数は1ミリも動かしていない）。★**Taka の残件7件は 済6／部分1／未0**。★**朝は済0**——『進んだ』ではなく**『測ったら済んでいたものが在った』**（測って分かった4件・直っていた1件・決めた1件）。★**併記「先頭が同じでも別の依頼です」が同じ日に効いた**——初めて出した2件の先頭30字が同一で、全文は 1092字 と 4089字 の別の依頼だった）（★2026-07-30 D-76/D-78 同期: DW の `derive_state`/`completion_blockers`/`next_legal_operation` に**省略可能な `events=` を1つ足し**、`submit._active_2der_tasks` が既に1回読んでいたものを渡すようにした——**目的は速くすることではなく「答えを変えずに読み直しをやめる」ことであり、155件の分類が変更前後で差分0行・引数を省略した版でも差分0行**。★CC-α は IMPL の基準を使わず **git HEAD から変更前のコードを取り出して自分で基準を作った**＝D-59 の独立検算の最初の1件。★表示の母数は `_active_2der_tasks` が返す集合（実測153件）で **`BLOCKED` 2件 はその外**（`COMPLETE` は実例0件で動きは未確認）。★受入5 は CC-α に再現できず **G-74** として登記）（★2026-07-28 D-41 で G-33〜G-35 を追加し、MD 表を JSON と同期した——それまで MD 表は G-15 で止まっており、§6 が自ら宣言した不変条件『`gaps[].id` の集合が MD §5.11 の表と一致する』が破れていた。★この不変条件を検査する計器は存在しない）。★未達=commit 記録／EventStore 等の責務比較／構想との差分／機械可読版／Taka への提示。調査の大半は静的であり、`【読】` を `LIVE` に昇格させていない。*
