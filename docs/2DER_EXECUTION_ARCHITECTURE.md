...
| **G-74** | **非回帰98本の走らせ方が★版管理された場所に書かれていない**（走らせ方は `.cc_tmp/` の外部スクリプトにしか無い。★**「実装者以外の誰も再現できない」は誤りだった**——CC-α が自分の3行で 91/7 と失敗7本の顔ぶれを再現した） | 作法の欠陥 | **PARTIALLY_CLOSED**（★残るのは「`.cc_tmp` が消えれば同じことが起きる」。★D-83 により**今は直さず登記して進む**） |
