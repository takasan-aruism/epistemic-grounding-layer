...
print(f"### Registration Acceptance Test SGQ-R1..R6 (corpus={len(recs)}) ###\n")
out = []
for qid, q in SGQ:
    ans, hits, raw = SG.answer_question(q, recs, sup, k=10, record_char_limit=1800, max_tokens=1400)
    v = SG.validate_answer(ans, ids)
    print(f"── {qid}: {q}")
    print(f"   retrieved={hits[:6]}")
    if ans:
    ...
    print()
    out.append({"sgq": qid, "question": q, "answer": ans, "validate": {k: v[k] for k in ("ok", "problems", "coercions")}, "metrics": v["metrics"]})
