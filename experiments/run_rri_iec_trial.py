...
real RRI slice(validate_iec, §10）を DW loop に、sequential swap で流す:
...
# ── RRI slice: validate_iec (§10) ────────────────────────────────────────────
IEC_SPEC = (
    "Implement `validate_iec(iec, known_refs)` for an RRI Interpretation Explanation Contract. `iec` should be a "
    "dict with: selected_interpretation (str), strategy (one of DIRECT, CONTEXT_RESOLVE, CHOICE, BOUNDED_MULTI_VIEW, "
    "INTENT_PROBE, PREMISE_PROBE, DEFER), basis_refs (list of str), residual (list), alternatives (list), "
    "revision_triggers (list). `known_refs` is a list/set of valid context ref ids. Return a dict {\"ok\": bool, "
    "\"problems\": list}. RULES: (R1) all required fields present else ok False. (R2) strategy must be one of the 7 "
    "else ok False. (R3) EVERY basis_ref must be in known_refs; a basis_ref not in known_refs is FABRICATION -> ok "
    "False (never allow a fabricated basis). (R4) C-TOTALITY: if iec is not a dict, or a field has the wrong type, "
    "return {ok False, problems:[...]} — NEVER crash and NEVER return ok True on malformed input. "
    "Return ONLY JSON {\"source\": \"<full python source of validate_iec>\"}."
)
KNOWN = ["CTX-1", "CTX-2", "CTX-3"]
GOOD = {"selected_interpretation": "X", "strategy": "CONTEXT_RESOLVE", "basis_refs": ["CTX-1"], "residual": [], "alternatives": [], "revision_triggers": ["USER_CORRECTION"]}
CASES = [
    (GOOD, KNOWN, True, "valid"),
    ({**GOOD, "basis_refs": ["CTX-9"]}, KNOWN, False, "fabricated basis_ref -> reject (R3)"),
    ({k: v for k, v in GOOD.items() if k != "residual"}, KNOWN, False, "missing field (R1)"),
    ({**GOOD, "strategy": "BOGUS"}, KNOWN, False, "bad strategy (R2)"),
    (None, KNOWN, False, "non-dict -> no crash (R4)"),
    ({**GOOD, "basis_refs": "CTX-1"}, KNOWN, False, "wrong-type field (R4)"),
]
...
def extract(raw):
    ...
    if 0 <= i < j:
        try:
            ...
            if isinstance(o.get("source"), str) and "def validate_iec" in o["source"]:
                return o["source"]
        except Exception:
            pass
    m = re.search(r"(def validate_iec.*)", raw, re.S)
...
def run_tests(source):
    ...
    harness = source + "\n\nimport json\n_c=" + repr(CASES) + "\n" + r"""
_r=[]
for iec,known,exp,lbl in _c:
    try:
        out=validate_iec(iec,known)
...
def main():
    trial = {"mode": "A (sequential swap)", "task": "RRI validate_iec (§10)", "swaps": SWAPS}
    final_source = None
    try:
        ...
        with tempfile.TemporaryDirectory() as dd:
            ...
            import importlib
            importlib.reload(W)
            TASK, mgr = "TASK-RRI-IEC-01", "claude-manager"
            tk = [0]
            def ts():
                tk[0] += 1
            ...
            W.create_task(TASK, "RRI", "implement validate_iec (§10, anti-fabrication + C-TOTALITY)", kp, ts(), mgr)
            W.record_plan(TASK, {"narrow_goal": "validate_iec: R1 fields / R2 strategy enum / R3 no fabricated basis_ref / R4 C-TOTALITY"}, ts(), mgr)

            # GENERATE (Coder-Next)
            assert swap_to("coder")[0], "coder swap failed"
            raw = chat(CODER_EP, CODER_M, "You are a careful Python coding worker. Implement EXACTLY the spec.", IEC_SPEC, seed=3)
            ...
            while True:
                ...
                raw = chat(CODER_EP, CODER_M, "Fix ALL findings; resubmit FULL corrected source.", IEC_SPEC + "\nPRIOR FAILED:\n" + fb, seed=3 + rework + 1)
                final_source = extract(raw)
                tr = run_tests(final_source)
            ...
            if state == "READY_FOR_UPPER_REVIEW":
                ...
                try:
                    pkt = W.propose_complete(TASK, [{"observed": f"validate_iec passed {tr.get('n_pass')}/{tr.get('cases')} load-bearing tests"}],
                                             [{"proposed": "RRI validate_iec (§10) implemented; passes tested anti-fabrication + C-TOTALITY cases", "scope": "tested cases only", "record_ids": []}], [], ts(), mgr)
        ...
        Path("/home/takasan/egl/experiments/rri_iec_trial.json").write_text(json.dumps(trial, ensure_ascii=False, indent=2))
        print(f"\n=== instrumentation ===")
