...
real RRI slice(approved_rq_set_gate, §29）を DW loop に、sequential swap で流す:
...
# ── RRI slice: approved_rq_set_gate (§29) — sanctioned path / no research bypass ──────
SLICE_SPEC = (
    "Implement `approved_rq_set_gate(rq_set)` for an RRI Approved RQ Set gate (spec section 29). It decides whether "
    "an RQ set may proceed to a SearchPlan. rq_set is a dict. Return {\"may_proceed\": bool, \"reason\": str}. "
    "RULES: (G1) C-TOTALITY: if rq_set is not a dict, return may_proceed False (never crash). (G2) approval_status "
    "must equal 'APPROVED' else may_proceed False. (G3) SANCTIONED PATH: source_research_design_id AND "
    "source_revision_id must BOTH be present and non-empty (this proves the set went through Research Design + Audit "
    "+ Revision); if either is missing or empty, may_proceed False — a set without design+revision lineage is a "
    "bypass and MUST be rejected. (G4) required_rqs must be a non-empty list else may_proceed False. (G5) at least "
    "one of resolved_intent_ref or validated_need_ref must be present and non-empty (the set must trace to a "
    "resolved intent or a validated need, never a raw request) else may_proceed False. "
    "Return ONLY JSON {\"source\": \"<full python source of approved_rq_set_gate>\"}."
)
GOOD = {"approval_status": "APPROVED", "source_research_design_id": "RDES-1", "source_revision_id": "RDRV-1",
        "required_rqs": ["RQ-1"], "resolved_intent_ref": "RINT-1"}
CASES = [
    (GOOD, True, "valid approved set -> may proceed"),
    ({**GOOD, "source_revision_id": ""}, False, "missing revision (no audit) -> reject bypass (G3)"),
    ({**GOOD, "source_research_design_id": None}, False, "missing design lineage -> reject bypass (G3)"),
    ({**GOOD, "approval_status": "DRAFT"}, False, "not APPROVED -> reject (G2)"),
    ({**GOOD, "required_rqs": []}, False, "empty required_rqs -> reject (G4)"),
    ({k: v for k, v in GOOD.items() if k != "resolved_intent_ref"}, False, "no intent/need ref -> reject raw bypass (G5)"),
    ({**{k: v for k, v in GOOD.items() if k != "resolved_intent_ref"}, "validated_need_ref": "RNEED-1"}, True, "validated_need_ref alternative -> may proceed"),
    (None, False, "non-dict -> no crash (G1)"),
]
...
def extract(raw):
    ...
    if 0 <= i < j:
        try:
            ...
            if isinstance(o.get("source"), str) and "def approved_rq_set_gate" in o["source"]:
                return o["source"]
        except Exception:
            pass
    m = re.search(r"(def approved_rq_set_gate.*)", raw, re.S)
...
def run_tests(source):
    ...
    harness = source + "\n\nimport json\n_c=" + repr(CASES) + "\n" + r"""
_r=[]
for rq_set,exp,lbl in _c:
    try:
        out=approved_rq_set_gate(rq_set)
...
def main():
    trial = {"mode": "A (sequential swap)", "task": "RRI approved_rq_set_gate (§29)", "swaps": SWAPS}
    final_source = None
    try:
        ...
        with tempfile.TemporaryDirectory() as dd:
            ...
            import importlib
            importlib.reload(W)
            TASK, mgr = "TASK-RRI-RQGATE-01", "claude-manager"
            tk = [0]
            def ts():
                tk[0] += 1
            ...
            W.create_task(TASK, "RRI", "implement approved_rq_set_gate (§29, sanctioned path / no research bypass)", kp, ts(), mgr)
            W.record_plan(TASK, {"narrow_goal": "approved_rq_set_gate: G2 APPROVED / G3 design+revision lineage (no bypass) / G4 required_rqs / G5 intent-or-need ref / G1 C-TOTALITY"}, ts(), mgr)
            ...
            if state == "READY_FOR_UPPER_REVIEW":
                ...
                try:
                    pkt = W.propose_complete(TASK, [{"observed": f"approved_rq_set_gate passed {tr.get('n_pass')}/{tr.get('cases')} load-bearing tests"}],
                                             [{"proposed": "RRI approved_rq_set_gate (§29) implemented; passes tested sanctioned-path/no-bypass + C-TOTALITY cases", "scope": "tested cases only", "record_ids": []}], [], ts(), mgr)
        ...
        Path("/home/takasan/egl/experiments/rri_rqgate_trial.json").write_text(json.dumps(trial, ensure_ascii=False, indent=2))
        print(f"\n=== instrumentation ===")
