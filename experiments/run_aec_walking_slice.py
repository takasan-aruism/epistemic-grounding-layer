...
PACKETS = [
 ...
 {"answer_claim_id": "AEC-AC-4",
  "statement": "現在の構成(Qwen3.6-NVFP4/tp2/sm_120)では vLLM Sleep Mode の wake_up が失敗するため、swap 代替として使えません(sleep 自体は 5.5s で ~28GB/GPU を解放するが wake が壊れている)。",
  "claim_class": "FACT", "basis_kind": "LOCAL_MEASUREMENT", "validation_mode": "MEASURED",
  "egl_claim_ref": None, "evidence_refs": ["sleep_wake_measured.json", "DE-0086"],
  "evidence_summary": "実測: sleep L1=5.5s(GPU 30GB→2GB/GPU)、wake_up=HTTP500 \"'list' object has no attribute 'zero_'\"、post-wake 生成 timeout",
  "source_policy": "LOCAL_OPERATIONAL", "source_role": "LOCAL_MEASUREMENT",
  "supports_dimension": "current-config Sleep Mode viability (measured)",
  "unsupported_dimension": "other models / configs",
  "scope": "現在の Qwen3.6-35B-A3B-NVFP4 / tp2 / expert-parallel / sm_120 config",
  "currentness": "2026-07-07", "local_applicability": "MEASURED",
  "claim_registration_status": "NOT_ADMITTED",
  "residuals": ["別 model / config では未測", "wake の list.zero_ error は Qwen3-Next-family 固有の可能性",
                "Sleep Mode は不可 → batching(AC-3)が残る viable path"]},
