...
def main(argv):
    ...
    print("  %-4s %-12s %-2s %-9s %-12s %8s %7s %-17s %6s %6s %-8s"
          % ("id", "領域", "Lv", "口", "期待", "実測chars", "秒", "本線の語", "本文率", "URL密", "材料"))
    print("  " + "-" * 118)
    for cid, area, lv, kind, target, expect, basis in CASES:
        ...
        _q = r.get("quality") or {}
        print("  %-4s %-12s %-2d %-9s %-12s %8s %6.1fs %-17s %6s %6s %-8s %s"
              % (cid, area, lv, kind, expect, got, sec,
                 ...
                 (_q.get("material_kind") or {}).get("kind") or "-",
