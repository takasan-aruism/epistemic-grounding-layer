...
def material_quality(text):
    ...
    ★★★2026-09-01 実測で 分かった こと= ★`body_ratio` は ★索引(リンク一覧)と 本文を
      ★分けられません。★使わないで ください。
      ★私の実測(分母 16= 索引8 / 本文8)= ★索引 0.0〜84.2% ／ 本文 26.1〜86.8%= ★完全に 重なる。
      ★Inference Control の 独立した 実測(分母 8)= ★索引 68.1〜82.1% ／ 本文 29.2〜38.1%=
        ★★向きが 逆(索引の方が 高い)。★理由= 検索の 題名は 短くても 句点を 含む 割合が 高く、
        ★本文は 見出し・箇条書き・脚注が 混ざる ∴ 本文率が 下がる。
      ★★∴ `body_ratio` は ★材料の 説明としては 残す が ★判別には 使わない。
    ★★★分けられるのは `url_density` の 方= ★私の実測 索引 1.000〜1.000 ／ 本文 0.000〜0.013=
      ★重なり 0。★Inference Control の 実測でも 索引 1.000 ／ 本文 0.000〜0.003= ★重なり 0。
      ★★閾値は ★私たちでは 決めません(★二人で そう決めた)= ★MGR の 裁定へ 出す。
        ★材料= 隙間は 0.013〜1.000 ／ 中点は 約 0.507 ／ ★分母は 合わせて 24件。
    """
    import re as _re
