...
real RRI slice(validate_resolved_intent, §11）を DW loop に、sequential swap で流す:
...
# ── RRI slice: validate_resolved_intent (§11) — anti-fabrication + don't-lose-residual ──
SLICE_SPEC = (
    "Implement `validate_resolved_intent(rint, known_iec_refs, known_request_refs)` for an RRI RESOLVED_INTENT "
    "(spec section 11). rint is a dict with: resolved_intent_id (str), request_id (str), primary_intent (str), "
    "intent_status (one of RESOLVED, RESOLVED_WITH_RESIDUAL, MULTI_INTENT, CLARIFICATION_REQUIRED, "
    "PREMISE_UNRESOLVED, DEFERRED), strategy (one of DIRECT, CONTEXT_RESOLVE, CHOICE, BOUNDED_MULTI_VIEW, "
    "INTENT_PROBE, PREMISE_PROBE, DEFER), iec_ref (str), and optional retained_intent_branches (list). "
    "known_iec_refs and known_request_refs are lists of valid ids. Return {\"ok\": bool, \"problems\": list}. "
    "RULES: (R1) all required fields (resolved_intent_id, request_id, primary_intent, intent_status, strategy, "
    "iec_ref) present else ok False. (R2) intent_status must be one of the 6 values else ok False. (R3) strategy "
    "must be one of the 7 values else ok False. (R4) iec_ref must be in known_iec_refs; not in it is FABRICATION -> "
    "ok False. (R5) request_id must be in known_request_refs (it must trace to a real raw request) else ok False. "
    "(R6) C-TOTALITY: if rint is not a dict, or a field has the wrong type, return ok False; NEVER crash and NEVER "
    "ok True on malformed input. (R7) if intent_status is RESOLVED_WITH_RESIDUAL or MULTI_INTENT, "
    "retained_intent_branches must be a non-empty list (a residual/multi-intent must NOT drop its retained "
    "branches) else ok False. Return ONLY JSON {\"source\": \"<full python source of validate_resolved_intent>\"}."
)
GOOD = {"resolved_intent_id": "RINT-1", "request_id": "REQ-1", "primary_intent": "X", "intent_status": "RESOLVED", "strategy": "CONTEXT_RESOLVE", "iec_ref": "IEC-1"}
CASES = [
    (GOOD, True, "valid RESOLVED"),
    ({**GOOD, "intent_status": "RESOLVED_WITH_RESIDUAL", "retained_intent_branches": ["B1"]}, True, "RESOLVED_WITH_RESIDUAL + retained branches"),
    ({**GOOD, "iec_ref": "IEC-9"}, False, "fabricated iec_ref (R4)"),
    ({**GOOD, "request_id": "REQ-9"}, False, "fabricated request_id (R5)"),
    ({**GOOD, "intent_status": "BOGUS"}, False, "bad intent_status (R2)"),
    ({**GOOD, "strategy": "BOGUS"}, False, "bad strategy (R3)"),
    ({k: v for k, v in GOOD.items() if k != "primary_intent"}, False, "missing field (R1)"),
    ({**GOOD, "intent_status": "RESOLVED_WITH_RESIDUAL"}, False, "RESOLVED_WITH_RESIDUAL but no retained branches (R7 lost residual)"),
    (None, False, "non-dict (R6)"),
]
...
def extract(raw):
    ...
    if 0 <= i < j:
        try:
            ...
            if isinstance(o.get("source"), str) and "def validate_resolved_intent" in o["source"]:
                return o["source"]
        except Exception:
            pass
    m = re.search(r"(def validate_resolved_intent.*)", raw, re.S)
...
def run_tests(source):
    ...
    harness = source + "\n\nimport json\n_c=" + repr(CASES) + "\n" + r"""
_r=[]
for rint,exp,lbl in _c:
    try:
        out=validate_resolved_intent(rint, ["IEC-1"], ["REQ-1"])
...
def main():
    trial = {"mode": "A (sequential swap)", "task": "RRI validate_resolved_intent (§11)", "swaps": SWAPS}
    final_source = None
    try:
        ...
        with tempfile.TemporaryDirectory() as dd:
            ...
            import importlib
            importlib.reload(W)
            TASK, mgr = "TASK-RRI-RINT-01", "claude-manager"
            tk = [0]
            def ts():
                tk[0] += 1
            ...
            W.create_task(TASK, "RRI", "implement validate_resolved_intent (§11, anti-fabrication + dont-lose-residual + C-TOTALITY)", kp, ts(), mgr)
            W.record_plan(TASK, {"narrow_goal": "validate_resolved_intent: R1 fields / R2 status enum / R3 strategy enum / R4-R5 no fabricated iec/request ref / R7 keep retained branches on residual / R6 C-TOTALITY"}, ts(), mgr)
            ...
            if state == "READY_FOR_UPPER_REVIEW":
                ...
                try:
                    pkt = W.propose_complete(TASK, [{"observed": f"validate_resolved_intent passed {tr.get('n_pass')}/{tr.get('cases')} load-bearing tests"}],
                                             [{"proposed": "RRI validate_resolved_intent (§11) implemented; passes tested anti-fabrication + dont-lose-residual + C-TOTALITY cases", "scope": "tested cases only", "record_ids": []}], [], ts(), mgr)
        ...
        Path("/home/takasan/egl/experiments/rri_rint_trial.json").write_text(json.dumps(trial, ensure_ascii=False, indent=2))
        print(f"\n=== instrumentation ===")
