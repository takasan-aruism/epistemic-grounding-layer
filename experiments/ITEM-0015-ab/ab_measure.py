...
success, over 3 round-trips. NO success unless both regenerate each cycle. approval APPROVAL-f055c945da."""
...
def sleep_model(url):
    t=time.time(); _post(url, "/sleep?level=2", timeout=120)
    for _ in range(120):
...
log={"approval_id":"APPROVAL-f055c945da","round_trips":[],"errors":[]}
