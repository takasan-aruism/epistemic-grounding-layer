...
# R6/DE-0025: Observation kind(F3a の Phase 1a サブセット)。source_class(権威次元)だけでは
...
def mk_observation(run, source_id, section_heading, blocks, observation_kind="UNSPECIFIED"):
    # R6/DE-0025: observation_kind は validation_mode 導出の一次資料(source_class と別次元)。
    # 既定 UNSPECIFIED は「種別未宣言」= mode を DECLARED/SPECIFIED へ格上げしない安全側。
    if observation_kind not in OBSERVATION_KINDS:
        raise ValueError(f"R6: unknown observation_kind {observation_kind!r} (allowed={sorted(OBSERVATION_KINDS)})")
